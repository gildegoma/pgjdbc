...
public class PGCopyOutputStream extends OutputStream implements CopyIn
{
  ...
  {
    if (at > 0
        && siz > copyBuffer.length - at)
    { // would not fit into rest of our buf, so flush buf
      op.writeToCopy(copyBuffer, 0, at);
      at = 0;
    }
    if (siz > copyBuffer.length)
    { // would still not fit into buf, so just pass it through
      op.writeToCopy(buf, off, siz);
    } else
    { // fits into our buf, so save it there
