 ...
 * to execute the query efficiently. <p> Query objects should be explicitly closed when no longer
 * needed; if resources are allocated on the server for this query, their cleanup is triggered by
 * closing the Query.
 *
...
public interface Query
{
   ...
   * object. If this query does have parameters, the returned ParameterList is a new list, unshared
   * by other callers.
   *
   ...
   * be immediately deallocated, but closing a Query may make the deallocation more prompt. <p> A
   * closed Query should not be executed.
   */
  void close();
