 ...
 * protocol-independent JDBC implementation code. <p> In general, a single query execution will
 * consist of a number of calls to handleResultRows, handleCommandStatus, handleWarning, and
 * handleError, followed by a single call to handleCompletion when query execution is complete. If
 * the caller wants to throw SQLException, this can be done in handleCompletion. <p> Each executed
 * query ends with a call to handleResultRows, handleCommandStatus, or handleError. If an error
 * occurs, subsequent queries won't generate callbacks.
 *
...
public interface ResultHandler
{
   ...
   * @param cursor    a cursor to use to fetch additional data; <code>null</code> if no further
   *                  results are present.
   */
