...
/**
 * Abstracts the protocol-specific details of executing a query. <p> Every connection has a single
 * QueryExecutor implementation associated with it. This object provides:
 *
 * <ul> <li> factory methods for Query objects ({@link #createSimpleQuery} and {@link
 * #createParameterizedQuery}) <li> execution methods for created Query objects ({@link
 * #execute(Query, ParameterList, ResultHandler, int, int, int)} for single queries and {@link
 * #execute(Query[], ParameterList[], ResultHandler, int, int, int)} for batches of queries) <li> a
 * fastpath call interface ({@link #createFastpathParameters} and {@link #fastpathCall}). </ul>
 *
 * <p> Query objects may represent a query that has parameter placeholders. To provide actual values
 * for these parameters, a {@link ParameterList} object is created via a factory method ({@link
 * Query#createParameterList}). The parameters are filled in by the caller and passed along with the
 * query to the query execution methods. Several ParameterLists for a given query might exist at one
 * time (or over time); this allows the underlying Query to be reused for several executions, or for
...
public interface QueryExecutor
{
   ...
   *                   #createSimpleQuery(String)} or {@link #createParameterizedQuery(String)} on
   *                   this QueryExecutor object.
   ...
   *                   org.postgresql.core.Query#createParameterList()}.
   ...
   *
   * @param queries        the queries to execute; each must be a query returned from calling {@link
   *                       #createSimpleQuery(String)} or {@link #createParameterizedQuery(String)}
   *                       on this QueryExecutor object.
   * @param parameterLists the parameter lists for the queries. The parameter lists correspond 1:1
   *                       to the queries passed in the <code>queries</code> array. Each must be
   *                       non-<code>null</code> if the corresponding query takes parameters, and
   *                       must be a parameter object returned by {@link org.postgresql.core.Query#createParameterList()}
   ...
   * provided query string is not parsed for parameter placeholders ('?' characters), and the {@link
   * Query#createParameterList} of the returned object will always return an empty ParameterList.
   *
   ...
   * query string is parsed for parameter placeholders ('?' characters), and the {@link
   * Query#createParameterList} of the returned object will create an appropriately-sized
   * ParameterList.
   *
  ...
  /**
   * Create a new ParameterList implementation suitable for invoking a fastpath function via {@link
   * #fastpathCall}.
   *
   ...
   * @param params        a ParameterList returned from {@link #createFastpathParameters} containing
   *                      the parameters to pass to the backend function
